...
const sub_help_hook =
    ...
    \\   gitstore hook --gh-zsh       Print opt-in zsh wrapper for `gh`
    \\   gitstore hook --gh-bash      Print opt-in bash wrapper for `gh`
    \\   gitstore hook --gh-nu        Print opt-in nushell wrapper for `gh`
    \\
    \\OPTIONS:
    \\   --help, -h                   Show this help
    \\
...
const sub_help_gh_repo =
    \\NAME:
    \\   gitstore gh-repo — Print GH_REPO for GitHub CLI compatibility
    \\
    \\USAGE:
    \\   gitstore gh-repo             Resolve current directory
    \\   gitstore gh-repo <path>      Resolve a repo path or subdirectory
    \\   gitstore gh-repo --export    Print `export GH_REPO=...`
    \\
    \\OPTIONS:
    ...
    \\   under the configured ghq/gitstore root as host/owner/repo.
    \\
;
...
fn getGhqRoot(gpa: Allocator, io: Io) ![]u8 {
    const result = try exec_mod.exec(gpa, io, &.{ "ghq", "root" }, null);
    ...
    const trimmed = exec_mod.trimTrailingNewline(result.stdout);
...
fn cmdGhRepo(
    ...
    const configured_ghq_root = config_mod.loadRoot(gpa, io, environ_map) catch |err| switch (err) {
        error.OutOfMemory => return err,
        else => {
            var buf: [1024]u8 = undefined;
            var w = File.stderr().writerStreaming(io, &buf);
    ...
    defer gpa.free(configured_ghq_root);
    const ghq_root = realPathArg(gpa, io, configured_ghq_root) catch |err| switch (err) {
        error.FileNotFound => try gpa.dupe(u8, configured_ghq_root),
        error.OutOfMemory => return err,
        else => {
            var buf: [1024]u8 = undefined;
            var w = File.stderr().writerStreaming(io, &buf);
