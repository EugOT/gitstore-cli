...
test("release artifact canonicalization removes Darwin linker/cache noise", () => {
	...
	const b = syntheticMachO(0xbb, "fedcba9876543210fedcba9876543210", 0xdd);
...
test("hashDir frames canonicalized Mach-O release artifacts", async () => {
	...
	try {
		await Bun.write(
			join(aDir, "zt"),
			syntheticMachO(0x11, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"),
		);
		await Bun.write(
			join(bDir, "zt"),
			syntheticMachO(0x22, "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"),
		);
